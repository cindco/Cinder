//
//	InstagramOAuthHelper.cpp
//	Instagram OAuth Helper for Kaleidoscope
//
...
void InstagramOAuthHelperApp::keyDown(KeyEvent event)
{
    switch (event.getCode()) {
        ...
        case KeyEvent::KEY_c:
        case KeyEvent::KEY_C:
            InstagramStream::clearStoredTokens();
...
void InstagramOAuthHelperApp::startOAuthFlow()
{
    ...
    mAuthUrl = InstagramStream::getAuthUrl(mClientId, mRedirectUri);
...
void InstagramOAuthHelperApp::handleAuthorizationCode(const std::string& code)
{
    ...
    std::string accessToken = InstagramStream::exchangeCodeForToken(mClientId, mClientSecret, code, mRedirectUri);
